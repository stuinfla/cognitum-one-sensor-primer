Updated: 2026-06-14 14:00:00 EDT | Version 3.0.0
Created: 2026-06-12 15:30:00 EDT

# ruview-kb — RuView Repository Knowledge Base (RVF)

> ## ⬆️ CURRENT BUILD (2026-06-14, v3) — TWO VARIANTS, 7,174 vectors each
> This store now ships in **two versions built from the SAME passages** (so content never drifts):
> | File | Dims | Model | For |
> |---|---|---|---|
> | `ruview-kb.big.rvf` | **768** | `Xenova/bge-base-en-v1.5` (CLS pooling, asymmetric query prefix) | Mac/PC — sharper answers |
> | `ruview-kb.small.rvf` | **384** | `Xenova/all-MiniLM-L6-v2` | Cognitum One Seed — lighter |
>
> **7,174 vectors** per variant — `passages.jsonl` lines == `meta.json` entries == idmap entries (parity verified, 0 rejected). Full Rust **source bodies** (firmware + crates) are indexed, not just ADRs/docs. The query tool auto-selects the right embedder per `.rvf` via the `*.big.rvf.embed.json` sidecar; the big variant carries `rankScale` (0.45) for bge's tighter distance scale.
>
> **Everything BELOW describes the earlier v2 (384-only, 4,306-vector) build and is SUPERSEDED by this header** — kept for history/provenance. Trust the table above for current counts/models.
